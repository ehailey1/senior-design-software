...
{
	dev_err(&client->dev, "YOUREINGPIOINIT\n");
			   ...
			   struct ov7670_info *info)
{
	...
	//  	dev_err(dev, "Unsupported media bus type: %d\n",bus_cfg.bus_type);
	//  	return ret;
	//  }
			...
			const struct i2c_device_id *id)
{
	...
	struct ov7670_info *info;
	int ret;
	dev_err(&client->dev, "START_OF_PROBE\n");
	...
	if (IS_ERR(info->clk)) {
		dev_err(&client->dev, "ISERRINFO_XCLK\n");
		ret = PTR_ERR(info->clk);
		if (ret == -ENOENT){
			info->clk = NULL;
			dev_err(&client->dev, "ISERRINFO_XCLK_ENOENT\n");
		}
		else
			return ret;
	}
	...
	if (ret){
		dev_err(&client->dev, "PASSED_GPIO_INIT\n");
		return ret;
	}
	ov7670_power_on(sd);
	...
	if (info->clk) {
		...
		dev_err(&client->dev, "ur speed is %lu", clk_get_rate(info->clk));
		if (info->clock_speed < 10 || info->clock_speed > 48) {
			dev_err(&client->dev, "XCLK_IS_BAD\n");
			ret = -EINVAL;
			goto power_off;
		}
	}
	...
	if (ret) {
		v4l_dbg(1, debug, client,
			...
			dev_err(&client->dev, "CHIPNOTFOUND\n");
		goto power_off;
	}
	v4l_info(client, "chip found @ 0x%02x (%s)\n",
			client->addr << 1, client->adapter->name);
			dev_err(&client->dev, "OVFOUND\n");
