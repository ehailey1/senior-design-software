...
static int mx6s_start_streaming(struct vb2_queue *vq, unsigned int count)
{
	...
	vb = &buf->vb;
	vb->vb2_buf.state = VB2_BUF_STATE_ACTIVE;
		...
		int bufnum, bool err)
{
	...
	if (ibuf->discard) {
		...
		vb->sequence = csi_dev->frame_count;
		if (err)
				    ...
				    struct v4l2_format *f)
{
	...
	pix->pixelformat	= csi_dev->fmt->fourcc;
	pix->sizeimage		= f->fmt.pix.sizeimage;
	pix->field		= f->fmt.pix.field;
	csi_dev->type		= f->type;
			       ...
			       enum v4l2_buf_type i)
{
	...
	/* TAKEN FROM PHYTEC DRIVER VERSION was:
	 * ret = vb2_streamon(&csi_dev->vb2_vidq, i);
	 *if (!ret)
	 *	v4l2_subdev_call(sd, video, s_stream, 1);
	 * Stopped timeouts from occuring
			    ...
			    struct v4l2_async_subdev *asd)
{
	...
	if (csi_dev->asd.match.fwnode == subdev->dev->of_node)
		csi_dev->sd = subdev;
